...
/*   Updated: 2015/02/11 18:41:26 by jbrozzu          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		draw_case_one(t_coord *point, t_env *e)   // horizon gauche->droite ht/bas
{
	...
	while (x <= point->x2)
	{
		mlx_pixel_put(e->mlx, e->win, x, point->y1 + ((point->y2 - point->y1) * 
					(x - point->x1)) / (point->x2 - point->x1), 0xFF0000);	
		x++;
	}
}
...
void		draw_case_three(t_coord *point, t_env *e)  // vertical dr/gch gch/dr bas
{
	...
	while (x <= point->y2)
	{
		mlx_pixel_put(e->mlx, e->win, point->x1 + ((point->x2 - point->x1) * 
					(x - point->y1)) / (point->y2 - point->y1), x, 0xFF0000);
		x++;
	}
}
...
void	trace_line(t_coord *point, t_env *e)
{
	...
	else if ((positive_valor(point->x2 - point->x1)) < 
			(positive_valor(point->y2 - point->y1)))
	{
	...
	else
	{
		draw_case_one(point, e);
		draw_case_three(point, e);
	//	ft_putstr("la ligne n'est pas tracee...\n");
